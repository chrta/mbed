...
static uint32_t can_disable(can_t *obj) {
    uint32_t sm = LPC_C_CAN0->CANCNTL;
    LPC_C_CAN0->CANCNTL |= CANCNTL_INIT;
    return sm;
}

static inline void can_enable(can_t *obj) {
    if (LPC_C_CAN0->CANCNTL & CANCNTL_INIT) {
        LPC_C_CAN0->CANCNTL &= ~CANCNTL_INIT;
    }
}
...
int can_filter(can_t *obj, uint32_t id, uint32_t mask, CANFormat format, int32_t handle) {
    ...
    if(handle == 0) {
        uint32_t msgval = LPC_C_CAN0->CANMSGV1 | (LPC_C_CAN0->CANMSGV1 << 16);
    ...
    if(handle > 0 && handle < 32) {
        if(format == CANExtended) {
            ...
            LPC_C_CAN0->CANIF1_ARB1 = BFN_PREP(id, CANIFn_ARB1_ID);
            LPC_C_CAN0->CANIF1_ARB2 = CANIFn_ARB2_MSGVAL | CANIFn_ARB2_XTD | BFN_PREP(id >> 16, CANIFn_ARB2_ID);
            LPC_C_CAN0->CANIF1_MSK1 = BFN_PREP(mask, CANIFn_MSK1_MSK);
        ...
        LPC_C_CAN0->CANIF1_MCTRL = CANIFn_MCTRL_UMASK | CANIFn_MCTRL_EOB | CANIFn_MCTRL_RXIE | BFN_PREP(DLC_MAX, CANIFn_MCTRL_DLC);
        ...
        LPC_C_CAN0->CANIF1_CMDREQ = BFN_PREP(handle, CANIFn_CMDREQ_MN);
...
void can_irq_free(can_t *obj) {
    LPC_C_CAN0->CANCNTL &= ~CANCNTL_IE; // Disable Interrupts :)
...
void can_irq_set(can_t *obj, CanIrqType type, uint32_t enable) {
    ...
    can_disable(obj);
    if(enable == 0) {
        LPC_C_CAN0->CANCNTL &= ~(CANCNTL_IE | CANCNTL_SIE);
    }
    else {
        LPC_C_CAN0->CANCNTL |= CANCNTL_IE | CANCNTL_SIE;
    }
...
int can_config_rxmsgobj(can_t *obj) {
    ...
    for ( i = 0; i < MSG_OBJ_MAX; i++ )
    {
        ...
        // Start Transfer to given message number
        LPC_C_CAN0->CANIF1_CMDREQ = BFN_PREP(i, CANIFn_CMDREQ_MN);
...
void can_init(can_t *obj, PinName rd, PinName td) {
    ...
    LPC_SYSCON->PRESETCTRL1 |= PRESETCTRL1_CCAN_RST_N;
    LPC_SYSCON->SYSAHBCLKCTRL1 |= SYSAHBCLKCTRL1_CCAN;
    ...
    // Resume operation
    LPC_C_CAN0->CANCNTL &= ~CANCNTL_INIT;
    while ( LPC_C_CAN0->CANCNTL & CANCNTL_INIT );
...
int can_frequency(can_t *obj, int f) {
    ...
    if (btr > 0) {
        uint32_t cntl_init = LPC_C_CAN0->CANCNTL | CANCNTL_INIT;
        // Set the bit clock
        LPC_C_CAN0->CANCNTL |= CANCNTL_CCE | CANCNTL_INIT;
        LPC_C_CAN0->CANCLKDIV = clkdiv;
        LPC_C_CAN0->CANBT = btr;
        LPC_C_CAN0->CANBRPE = 0x0000;
        LPC_C_CAN0->CANCNTL &= ~(CANCNTL_CCE | CANCNTL_INIT);
        LPC_C_CAN0->CANCNTL |= cntl_init;
        return 1;
    }
    return 0;
}
...
int can_write(can_t *obj, CAN_Message msg, int cc) {
    ...
    if(msg.format == CANExtended)    {
        ...
        LPC_C_CAN0->CANIF1_ARB1 = BFN_PREP(msg.id, CANIFn_ARB1_ID);
        LPC_C_CAN0->CANIF1_ARB2 = CANIFn_ARB2_MSGVAL | CANIFn_ARB2_XTD | direction | BFN_PREP(msg.id >> 16, CANIFn_ARB2_ID);
        LPC_C_CAN0->CANIF1_MSK1 = BFN_PREP(ID_EXT_MASK, CANIFn_MSK1_MSK);
        LPC_C_CAN0->CANIF1_MSK2 = CANIFn_MSK2_MXTD | CANIFn_MSK2_MDIR | BFN_PREP(ID_EXT_MASK >> 16, CANIFn_MSK2_MSK);
    }
    else {
        // Mark message valid, Set Identifier and mask everything
        LPC_C_CAN0->CANIF1_ARB2 = CANIFn_ARB2_MSGVAL | direction | BFN_PREP(msg.id << 2, CANIFn_ARB2_ID);
        LPC_C_CAN0->CANIF1_MSK2 = CANIFn_MSK2_MDIR | BFN_PREP(ID_STD_MASK << 2, CANIFn_MSK2_MSK);
    }
    
    // Use mask, request transmission, single message object and set DLC
    LPC_C_CAN0->CANIF1_MCTRL = CANIFn_MCTRL_UMASK | CANIFn_MCTRL_TXRQST | CANIFn_MCTRL_EOB | BFN_PREP(msg.len, CANIFn_MCTRL_DLC);

    LPC_C_CAN0->CANIF1_DA1 = BFN_PREP(msg.data[1], CANIFn_DA1_DATA1) | BFN_PREP(msg.data[0], CANIFn_DA1_DATA0);
    LPC_C_CAN0->CANIF1_DA2 = BFN_PREP(msg.data[3], CANIFn_DA2_DATA3) | BFN_PREP(msg.data[2], CANIFn_DA2_DATA2);
    LPC_C_CAN0->CANIF1_DB1 = BFN_PREP(msg.data[5], CANIFn_DB1_DATA5) | BFN_PREP(msg.data[4], CANIFn_DB1_DATA4);
    LPC_C_CAN0->CANIF1_DB2 = BFN_PREP(msg.data[7], CANIFn_DB2_DATA7) | BFN_PREP(msg.data[6], CANIFn_DB2_DATA6);
    ...
    // Start Transfer to given message number
    LPC_C_CAN0->CANIF1_CMDREQ = BFN_PREP(msgnum, CANIFn_CMDREQ_MN);
    
    // Wait until transfer to message ram complete - TODO: maybe not block??
    while( LPC_C_CAN0->CANIF1_CMDREQ & CANIFn_CMDREQ_BUSY);
    
    // Wait until TXOK is set, then clear it - TODO: maybe not block
    //while( !(LPC_C_CAN0->CANSTAT & CANSTAT_TXOK) );
...
int can_read(can_t *obj, CAN_Message *msg, int handle) {
    ...
    if(handle > 0 && handle < 32) {
        ...
        LPC_C_CAN0->CANIF2_CMDREQ = BFN_PREP(handle, CANIFn_CMDREQ_MN);
        ...
        if (LPC_C_CAN0->CANIF2_ARB2 & CANIFn_ARB2_XTD) {
            ...
            msg->id = (LPC_C_CAN0->CANIF2_ARB1 & CANIFn_ARB2_ID_MASK) << 16;
            msg->id |= (LPC_C_CAN0->CANIF2_ARB2 & CANIFn_ARB2_ID_MASK);
        }
        else {
            msg->format = CANStandard;
            msg->id = (LPC_C_CAN0->CANIF2_ARB2 & CANIFn_ARB2_ID_MASK) >> 2;
        }
        ...
        msg->len        = BFN_GET(LPC_C_CAN0->CANIF2_MCTRL, CANIFn_MCTRL_DLC); // TODO: If > 8, len = 8
        msg->data[0]    = BFN_GET(LPC_C_CAN0->CANIF2_DA1, CANIFn_DA1_DATA0);
        msg->data[1]    = BFN_GET(LPC_C_CAN0->CANIF2_DA1, CANIFn_DA1_DATA1);
        msg->data[2]    = BFN_GET(LPC_C_CAN0->CANIF2_DA2, CANIFn_DA2_DATA2);
        msg->data[3]    = BFN_GET(LPC_C_CAN0->CANIF2_DA2, CANIFn_DA2_DATA3);
        msg->data[4]    = BFN_GET(LPC_C_CAN0->CANIF2_DB1, CANIFn_DB1_DATA4);
        msg->data[5]    = BFN_GET(LPC_C_CAN0->CANIF2_DB1, CANIFn_DB1_DATA5);
        msg->data[6]    = BFN_GET(LPC_C_CAN0->CANIF2_DB2, CANIFn_DB2_DATA6);
        msg->data[7]    = BFN_GET(LPC_C_CAN0->CANIF2_DB2, CANIFn_DB2_DATA7);
...
unsigned char can_rderror(can_t *obj) {
    return BFN_GET(LPC_C_CAN0->CANEC, CANEC_REC);
}

unsigned char can_tderror(can_t *obj) {
    return BFN_GET(LPC_C_CAN0->CANEC, CANEC_TEC);
}
